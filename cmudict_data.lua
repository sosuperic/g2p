...
local DICT_PATH = 'cmudict/cmudict.dict'

local NUM_VALID = 3500
local NUM_TEST = 15000
...
function split_line_into_word_and_phonemes(dict, i)
	...
	local word = line[1]
	local phonemes = subrange(line, 2, #line)
...
function get_data_and_graphemes()
	local dict = lines_from(DICT_PATH)
	local data = {}
	local graphemes = {}
	for i=1,#dict do
